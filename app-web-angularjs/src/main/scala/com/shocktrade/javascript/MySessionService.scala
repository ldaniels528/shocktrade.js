...
import com.shocktrade.javascript.models.contest.{Contest, Message, Participant}
...
class MySessionService($rootScope: Scope, $timeout: Timeout, toaster: Toaster,
  ...
  def getContestID = contest.orUndefined.flatMap(_._id)

  def getContestName = contest.flatMap(_.name.toOption).orNull

  def getContestStatus = contest.flatMap(_.status.toOption).orNull
  ...
  def deductFundsAvailable(amount: Double) = {
    participant foreach { player =>
      console.log(s"Deducting funds: $amount from ${player.cashAccount.flatMap(_.cashFunds)}")
      player.cashAccount.foreach(acct => acct.cashFunds = acct.cashFunds.map(_ - amount))
      // TODO rethink this
    }
    ()
  }

  def getMessages = contest.orUndefined.flatMap(_.messages.flat) getOrElse emptyArray[Message]

  def setMessages(messages: js.Array[Message]) = contest.foreach(_.messages = messages)

  def getMyAwards = userProfile.awards

  def getOrders = participant.orUndefined.flatMap(_.orders) getOrElse emptyArray

  def getClosedOrders = participant.orUndefined.flatMap(_.closedOrders) getOrElse emptyArray

  def getPerformance = participant.orUndefined.flatMap(_.performance) getOrElse emptyArray

  def getPerks = participant.orUndefined.flatMap(_.perks) getOrElse emptyArray

  def hasPerk(perkCode: String) = getPerks.contains(perkCode)

  def getPositions = participant.orUndefined.flatMap(_.positions) getOrElse emptyArray
  ...
  def findPlayerByName(contest: Contest, playerName: String) = {
    contest.participants.toOption.flatMap(_.find(_.name == playerName)) getOrElse New
  }
  ...
  private[javascript] def cashAccount_? = participant.flatMap(_.cashAccount.toOption)
  ...
  private[javascript] def participant: Option[Participant] = {
    for {
      ...
      c <- contest
      me <- c.participants.toOption.flatMap(_.find(_._id ?== userId))
    } yield me
  }

  private def lookupParticipant(contest: Contest, playerId: String) = {
    contest.participants.toOption.flatMap(_.find(_._id ?== playerId))
  }
  ...
  private def updateContestDelta(updatedContest: Contest) {
    ...
    for {
      ...
      myParticipantId <- myParticipant._id
