...
import com.shocktrade.controlpanel.Compiler._
import com.shocktrade.controlpanel.Tokenizer.{TokenEnrichment, TokenIterator}
...
import com.shocktrade.controlpanel.runtime.ops.{DefineFunctionOp, DefineVariableOp, MathOp, ScopeOp}
import com.shocktrade.controlpanel.runtime.values.{ArrayReference, FunctionReference, VariableReference}
...
class Compiler(tokenizer: Tokenizer) {
  ...
  /**
    ...
    */
  def arrayReference(it: TokenIterator) = {
    it.expect("[")
  ...
  /**
    ...
    */
  def defineFunction(it: TokenIterator) = {
  ...
  /**
    ...
    */
  def defineVariable(it: TokenIterator) = {
  ...
  /**
    ...
    */
  def functionReference(it: TokenIterator) = {
    val name = it.next()
    it.expect("(")
  ...
  /**
    ...
    */
  def variableReference(it: TokenIterator) = {
...
object Compiler {
  ...
  final implicit class TokenizerExtensions(val it: TokenIterator) extends AnyVal {
    ...
    def assert(f: TokenIterator => Boolean) = {
      it.peekOption match {
        case Some(token) =>
    ...
    def nextOption() = if (it.hasNext) Option(it.next()) else None

    def peekOption = if (it.hasNext) Some(it.peek) else None
