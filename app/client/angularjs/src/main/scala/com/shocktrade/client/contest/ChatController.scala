  ...
  $scope.getMessages = () => {
    ...
    if ((messages.length == lastMessageCount) && (js.Date.now() - lastUpdateTime) <= 1000) cachedHtml
      ...
      val html = messages.foldLeft[String]("") { (html, msg) =>
        ...
        html +
          s"""<img src="http://graph.facebook.com/${msg.sender.flatMap(_.facebookID)}/picture" class="chat_icon">
      ...
      case Some((playerId, facebookID, contestId)) =>
        if (messageText.trim.nonEmpty) {
          ...
          val message = new ChatMessage(
            sender = User(_id = playerId, facebookID = facebookID, name = mySession.getUserName),
            text = messageText
          )
