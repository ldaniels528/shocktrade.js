...
case class MySessionService($rootScope: Scope, $timeout: Timeout, toaster: Toaster,
  ...
  private[stockguru] def cashAccount_? = portfolio_?.flatMap(_.cashAccount.toOption)

  private[stockguru] def marginAccount_? = portfolio_?.flatMap(_.marginAccount.toOption)
