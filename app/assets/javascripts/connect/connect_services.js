 ...
 */
angular.module('shocktrade').factory('ConnectService', function($http, $q) {
	var service = {};
	...
	service.getUserInfo = function(fbUserId) {
		...
		.success(function(data, status, headers, config) { deferred.resolve(data[0]); })
		.error(function(data, status, headers, config) { deferred.reject("Error: " + symbol + "-" + data + "(" + status + ")"); });	
		return deferred.promise;
	};
