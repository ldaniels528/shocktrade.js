        ...
        function ($scope, $location, $log, $timeout, MySession, ContestService, Errors, InvitePlayerDialog, NewGameDialog, NewOrderDialog, QuoteService) {
            ...
            $scope.selectContest = function (contest) {
                ...
                if (!contest.rankings) {
                    ...
                    if(MySession.userProfile.id) {
                        loadEnrichedParticipant($scope.contestId, MySession.userProfile.id);
                    }
                }
            };

            function isContestSelected(contestId) {
                return ($scope.contest && $scope.contest.id == contestId);
            }
            ...
            $scope.$on("contest_deleted", function (event, response) {
                ...
                if (isContestSelected(response.id)) {
                    $scope.selectContest($scope.searchResults.length >= 1 ? $scope.searchResults[0] : null);
                }
            });
