...
angular
    .module('shocktrade')
    .controller('PlayCtrl', ['$scope', '$location', '$log', '$timeout', 'MySession', 'ContestService', 'Errors', 'InvitePlayerDialog', 'NewGameDialog', 'NewOrderDialog', 'QuoteService',
        function ($scope, $location, $log, $timeout, MySession, ContestService, Errors, InvitePlayerDialog, NewGameDialog, NewOrderDialog, QuoteService) {
            // setup UI variables
            $scope.statusBar = "";
            $scope.selection = {
                exposure: "sector",
            ...
            $scope.joinGame = function (participant) {
                $scope.joiningGame = true;
                var contestId = MySession.contestId;
                var playerId = MySession.userProfile._id.$oid;
                ...
                ContestService.joinContest(contestId, playerInfo)
                    .success(function () {
            ...
            $scope.joinedParticipant = function (contest, userProfile) {
                return containsPlayer(contest, userProfile);
            };
            ...
            function containsPlayer(contest, userProfile) {
                return userProfile._id && findPlayerByID(contest, userProfile._id.$oid) != null;
            }
            ...
            function selectContest(contest) {
                $scope.contest = contest;
                $scope.contestId = contest._id.$oid;
            }
            ...
            function indexOfContest(contestId) {
                for(var n = 0; n < $scope.searchResults.length; n++) {
                    ...
                    if(contest._id.$oid == contestId) return n;
                }
                return -1;
            }
             ...
             */
            $scope.$on("contest_deleted", function (event, response) {
                ...
                if(isContestSelected(response.id)) {
                    selectContest( $scope.searchResults.length >= 1 ? $scope.searchResults[0] : null);
                }
            });
             ...
             */
            $scope.$on("contest_updated", function (event, args) {
