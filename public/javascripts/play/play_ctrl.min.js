...
angular
	.module('shocktrade')
	.controller('PlayCtrl', ['$scope', '$location', '$log', '$modal', 'MySession', 'ContestService', 'Errors', 'QuoteService',
	                 function($scope, $location, $log, $modal, MySession, ContestService, Errors, QuoteService) {
	// setup UI variables
	$scope.statusBar = "";
	$scope.selection = {
		exposure: "sector",
	...
	$scope.changePlayTab = function(tabIndex, event) {
		console.log("Changing location to " + $scope.tabs[tabIndex].url);
		$location.path($scope.tabs[tabIndex].url);
	...
	$scope.setPlayTab = function(tabIndex, event) {
	...
	$scope.swithToPath = function(path) {
		$location.path(path);
	}
	...
	$scope.asOfDate = function(participant) {
		return participant.lastTradeTime ? participant.lastTradeTime.$date : new Date();
	}
	...
	$scope.getTotalInvestment = function(participant) {
		...
		for(var n = 0; n < positions.length; n++) {
			var p = positions[n];
			total += p.price * p.quantity + p.commision;
		}
		return total;
	};
	...
	$scope.gainLoss = function(tx) {
		...
		var comissions = (tx.commision1 || 0) + (tx.commision2 || 0) + (tx.commision || 0);
	...
	$scope.loadContest = function(contestId) {
		if(contestId && contestId != "") {
			...
			// load the contest
			ContestService.getContest(contestId).then(function(contest) { 
	...
	$scope.tropy = function(place) {
		switch(place) {
			case '1st': return "contests/gold.png";
			case '2nd': return "contests/silver.png";
			case '2nd': return "contests/bronze.png";
			default: return "status/transparent.png";
		}
	}
	...
	$scope.loadMyContests = function(userName) {
		if( userName ) {
			$scope.startLoading();
			$scope.message = "";
			ContestService.getMyContests(userName).then(
				function(data) {
					$scope.myContests = data;
					if(data.length > 0 && $scope.contestId == "") {
						$scope.contestId = data[0]._id.$oid;
	...
	$scope.$watch("MySession.userProfile", function(oldVal, newVal) {
		...
		if(playerName != 'Spectator') {
			...
			$scope.loadMyContests(playerName);
