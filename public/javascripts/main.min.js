...
angular.module('shocktrade')
	...
	$scope.isLoading = function() { return $scope._loading };
	...
	$scope.appInit = function() {
		...
		$interval(function() { $scope.marketClock = (new Date()).toTimeString(); }, 1000);
	...
	$scope.postLoginUpdates = function(fbUserID, userInitiated) {
		...
		Facebook.getFriends().then(
			function(response) {
				MySession.fbFriends = response.data;
			},
			function(err) {
	...
	function determineTab() {
		...
		else if (path.indexOf("/blog") != -1) tabIndex = 6;
		else if (path.indexOf("/admin") != -1) {
			if($scope.admin) tabIndex = 7; 
			else tabIndex = 0;
		}
		else tabIndex = 0;
	...
	$scope.$watch(function() { return MySession.userProfile; }, function() {		
		if( ! $scope.admin && MySession.userProfile && MySession.userProfile.name == "ldaniels" ) {
			$scope.admin = true;
			$scope.tabs.push({
				"name" : "Admin",
				"imageURL" : "/assets/images/objects/processes.png",
				"path" : "/admin",
				"active" : false
			});
		}
