...
object DateUtil {
   ...
   * @author lawrence.daniels@gmail.com
   */
  implicit class JavaDateEnichment(date: Date) {
   ...
   * @author lawrence.daniels@gmail.com
   */
  implicit class JodaTimeEnichment(date: DateTime) {

    def >(when: Date): Boolean = date.isAfter(when.getTime)

    def >=(when: Date): Boolean = date.equals(when) || date.isAfter(when.getTime)

    def <(when: Date): Boolean = date.isBefore(when.getTime)

    def <=(when: Date): Boolean = date.equals(when) || date.isBefore(when.getTime)
